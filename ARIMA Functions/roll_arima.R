roll_arima <- function(ts_data, start_date="2022-01-01", iter=1, timeframe=365,
                               ci.level=0.95, p=0, d=1, q=2, P=1, D=0, Q=2){
  ...
  while(i <= iter){
    ...
    asthma_one <- arima_fit(ts_data = asthma_train, lookahead = 1,
                            ci.level=ci.level, p=p, d=d, q=q, P=P, D=D, Q=Q)
    asthma_one
