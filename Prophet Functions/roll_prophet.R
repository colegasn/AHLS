roll_prophet <- function(ts_data, start_date="2022-01-01", iter=1, timeframe=365,
                     ci.level=0.95, include_history=FALSE){
  ...
  while(i <= iter){
    ...
    asthma_one <- prophet_fit(ts_data = asthma_train, lookahead = 1,
                          ci.level=ci.level, include_history = include_history)
    asthma_one
